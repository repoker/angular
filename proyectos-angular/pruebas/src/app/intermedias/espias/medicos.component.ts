...
export class MedicosComponent implements OnInit {
  ...
  agregarMedico() {
    ...
    this._medicoService.agregarMedico(medico).subscribe(medicoDB => this.medicos.push(medicoDB), err => this.mensajeError = err);
  }

  borrarMedico(id: string) {
    const confirmar = confirm('Estas seguro de querer borrar este médico');
    if (confirmar) {
